...
namespace MagicStorage
{
    public class MagicStorage : Mod
    {
        ...
        private void AddTranslations()
        {
            ...
            text = LocalizationLoader.CreateTranslation(this, "FilterAllMods");
            text.SetDefault("All mods");
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "全模组");
            LocalizationLoader.AddTranslation(text);

            text = LocalizationLoader.CreateTranslation(this, "CookedMarshmallowCondition");
            text.SetDefault("Biome Globe in a Crafting Interface");
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "存放于制作存储单元的生态球");
            LocalizationLoader.AddTranslation(text);

            text = LocalizationLoader.CreateTranslation(this, "Capacity");
            text.SetDefault("{0} / {1} Items");
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "容量: {0} / {1}");
            LocalizationLoader.AddTranslation(text);

            text = LocalizationLoader.CreateTranslation(this, "Inactive");
            text.SetDefault("Inactive");
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "已禁用");
            LocalizationLoader.AddTranslation(text);

            text = LocalizationLoader.CreateTranslation(this, "Active");
            text.SetDefault("Active");
            text.AddTranslation(GameCulture.FromCultureName(GameCulture.CultureName.Chinese), "已启用");
            LocalizationLoader.AddTranslation(text);
        }
